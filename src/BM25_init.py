...
class BM25Simple(object):
    ...
    def __init__(self, corpus):
        ...
        self.dl = [float(len(" ".join(d.replace("\xa0", "").replace("\n", "").replace("\\", "").split(" ")).split())) for d in self.corpus]
    ...
    def _initialize(self):
        for document in self.corpus:
            frequencies = {}
            words = " ".join(document.replace("\xa0", "").replace("\n", "").replace("\\", "").split()).split(" ")
            for word in words:
    ...
    def get_top_n(self, query, n=20):
        ...
        return [self.title[i] for i in indexes[:n]]

    def itemize(self, query):

        return " ".join(query.replace("\xa0", "").replace("\n", "").replace("\\", "").split()).split(" ")